...
function renderDoubanCards(data, container) {
    ...
    if (!data.subjects || data.subjects.length === 0) {
        ...
        data.subjects.forEach(item => {
            ...
            // 方案 1: 使用更稳定的 Weserv 地址格式，去掉 https 前缀有助于绕过部分检测
            const cleanUrl = originalCoverUrl.replace(/^https?:\/\//, '');
            const weservProxy = `https://images.weserv.nl/?url=${cleanUrl}&default=https://img3.doubanio.com/f/movie/30c62017c69990558197c2e2fe8694d6e9da9df3/pics/movie/movie_default_large.png`;
            
            // 方案 2: WordPress 代理（作为极佳的备选）
            ...
            card.innerHTML = `
                <div class="relative w-full aspect-[2/3] overflow-hidden cursor-pointer" onclick="fillAndSearchWithDouban('${safeTitle}')">
                    <img src="${weservProxy}" alt="${safeTitle}" 
                        ...
                        onerror="if(!this.getAttribute('data-tried-wp')){this.setAttribute('data-tried-wp', 'true'); this.src='${wpProxy}';}else{this.src='image/logo.png';}"
                        loading="lazy">
